...
import { ArrowLeft, TrendingUp, Award, Search } from 'lucide-react';
...
export default function MelhoresPage() {
  ...
  const melhores = indicadores
    .filter(i => i.indicadores.tendencia === 'melhora' || i.indicadores.scoreGeral >= 80)
    .filter(i => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-300">Colaboradores com melhor evolução e performance</p>
        </div>
      </div>
      ...
      <div className="card-white">
        <div className="card-white-header">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-white">Ranking de Melhores</h2>
            <span className="text-sm text-gray-300">{melhores.length} colaboradores</span>
          </div>
        </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className={`flex items-center justify-center w-10 h-10 rounded-full font-bold text-white border ${
                      index === 0 ? 'bg-yellow-500/20 border-yellow-500/50' : index === 1 ? 'bg-gray-500/20 border-gray-500/50' : index === 2 ? 'bg-orange-500/20 border-orange-500/50' : 'bg-blue-500/20 border-blue-500/50'
                    }`}>
                      {index < 3 ? (
                        index === 0 ? '🥇' : index === 1 ? '🥈' : '🥉'
                      ) : (
                  ...
                  <div className="flex items-center space-x-6">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-semibold text-white">{item.indicadores.totalAvaliacoes}</p>
                    </div>
                    <TrendingUp className="h-8 w-8 text-green-400" />
                  </div>
                </div>
                {item.indicadores.tendencia === 'melhora' && (
                  <div className="mt-3 pt-3 border-t border-blue-500/30">
                    <div className="flex items-center text-sm text-green-400">
                      <TrendingUp size={16} className="mr-1" />
                      <span className="font-medium">Tendência de melhora identificada</span>
                    </div>
                  </div>
                )}
              </div>
